...
const SOC01_SPEC_DIAGRAMS = [
  {
    src: '/images/products/small-office-cabin/specs/small-office-cabin-envelope-coordination-diagram.webp',
    alt: smallOfficeCabinCopy.alt_text.spec_diagram_1,
    width: 1600,
    height: 900,
  },
  {
    src: '/images/products/small-office-cabin/specs/small-office-cabin-opening-electrical-coordination-diagram.webp',
    alt: smallOfficeCabinCopy.alt_text.spec_diagram_2,
    width: 1600,
    height: 771,
  },
] as const;
const SOC01_SPEC_PDF = {
  href: '/downloads/small-office-cabin-technical-specification-and-price.pdf',
  label: 'Download technical specification (PDF)',
} as const;
...
// SOC-01 (4 Sep 2026) — Small Office Cabin Specifications tab. Groups A-E are
// rendered as the same premium grouped-table cards the porta-cabins Specs tab
// uses (saman-table-wrap / saman-table, the identical card chrome), generalised
// only to accept each group's own column count. Every string comes from the
// signed copy pack; nothing is retyped here. Each group's note renders directly
// under its own table, then the two coordination diagrams, then the PDF link.
function buildSmallOfficeCabinSpecificationsHtml(): string {
  const spec = smallOfficeCabinCopy.specifications_tab;
  ...
  const diagrams = SOC01_SPEC_DIAGRAMS
    .map((d) => (
      `<figure class="mt-4 m-0 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">` +
  ...
  const pdf =
    `<p class="mt-5 text-sm"><a href="${esc(SOC01_SPEC_PDF.href)}" ` +
    `class="font-semibold text-emerald-800 underline underline-offset-2">` +
    `${esc(SOC01_SPEC_PDF.label)}</a></p>`;

  return `<div class="not-prose">${narrative}${cards}${diagrams}${pdf}</div>`;
}

// PO-01 - the two approved technical diagrams and the v3 technical PDF, at the exact
// dimensions of the shipped WebP derivatives (never cropped, never upscaled). Diagram 1
// is the package-defect source whose colliding "Engineering boundary:" label was
// repaired (label re-set with a word space) before conversion; the drawing is untouched.
const PO01_SPEC_DIAGRAMS = [
  {
    src: '/images/products/readymade-office-cabin/diagrams/readymade-office-cabin-frame-joint-diagram.webp',
    alt: readymadeOfficeCabinCopy.alt_text.spec_diagrams['diagrams/readymade-office-cabin-frame-joint-diagram.webp'],
    width: 1600,
    height: 900,
  },
  {
    src: '/images/products/readymade-office-cabin/diagrams/readymade-office-cabin-window-electrical-diagram.webp',
    alt: readymadeOfficeCabinCopy.alt_text.spec_diagrams['diagrams/readymade-office-cabin-window-electrical-diagram.webp'],
    width: 1600,
    height: 900,
  },
] as const;
const PO01_SPEC_PDF = {
  href: '/images/products/readymade-office-cabin/docs/readymade-office-cabin-technical-specification-and-price-v3.pdf',
  label: readymadeOfficeCabinCopy.specifications_tab.pdf_link_label,
} as const;

// PO-01 (4 Sep 2026) - Readymade Office Cabin Specifications tab. Groups A-E render as
// the same premium grouped-table cards the porta-cabins Specs tab uses (identical card
// chrome, saman-table-wrap / saman-table), generalised only to accept each group's own
// column count. Every string comes from the signed copy pack; nothing is retyped here.
// Each group's note renders directly under its own table, then the two diagrams, then
// the PDF link. The live page carried 0 tables, 0 diagrams and 0 links.
function buildReadymadeOfficeCabinSpecificationsHtml(): string {
  const spec = readymadeOfficeCabinCopy.specifications_tab;
  ...
  const cards = spec.groups.map((group) => {
    const head = group.head.map((h) => `<th>${esc(h)}</th>`).join('');
    const body = group.rows
      .map((row) => (
        `<tr>` +
          row.map((cell, i) => (
  ...
  const diagrams = PO01_SPEC_DIAGRAMS
    .map((d) => (
      `<figure class="mt-4 m-0 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">` +
        `<img src="${esc(d.src)}" alt="${esc(d.alt)}" width="${d.width}" height="${d.height}" ` +
        `loading="lazy" class="w-full h-auto rounded-lg" />` +
  ...
  const pdf =
    `<p class="mt-5 text-sm"><a href="${esc(PO01_SPEC_PDF.href)}" ` +
    `class="font-semibold text-emerald-800 underline underline-offset-2">` +
    `${esc(PO01_SPEC_PDF.label)}</a></p>`;

  return `<div class="not-prose">${narrative}${cards}${diagrams}${pdf}</div>`;
}

// PO-02 - the two approved technical diagrams and the v1 twelve-page technical PDF, at
// the exact dimensions of the shipped WebP derivatives (never cropped, never upscaled).
// The PDF keeps the public path the repo already serves for this page; only the file
// behind it is replaced.
const PO02_IMG_ROOT = `/${prefabricatedOfficeCabinsAssets.output_root.replace('public/', '')}`;
const PO02_SPEC_DIAGRAMS = [
  {
    src: `${PO02_IMG_ROOT}/${prefabricatedOfficeCabinsAssets.spec_diagrams['1'].out}`,
    alt: prefabricatedOfficeCabinsCopy.alt_text.spec_diagram_1,
    width: 1600,
    height: 900,
  },
  {
    src: `${PO02_IMG_ROOT}/${prefabricatedOfficeCabinsAssets.spec_diagrams['2'].out}`,
    alt: prefabricatedOfficeCabinsCopy.alt_text.spec_diagram_2,
    width: 1600,
    height: 900,
  },
] as const;
const PO02_SPEC_PDF = {
  href: '/specs/prefabricated-office-cabins-technical-specification.pdf',
  label: prefabricatedOfficeCabinsAssets.spec_pdf.link_label,
} as const;

// PO-02 (4 Sep 2026) - Prefabricated Office Cabins Specifications tab. Groups A-E
// render as the same premium grouped-table cards the porta-cabins Specs tab uses
// (identical card chrome, saman-table-wrap / saman-table), generalised only to accept
// each group's own column count. Every string comes from the signed copy pack; nothing
// is retyped here. Each group's note renders directly under its own table, then the two
// diagrams, then the PDF link.
function buildPrefabricatedOfficeCabinsSpecificationsHtml(): string {
  const spec = prefabricatedOfficeCabinsCopy.specifications_tab;
  ...
  const diagrams = PO02_SPEC_DIAGRAMS
    .map((d) => (
      `<figure class="mt-4 m-0 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">` +
        `<img src="${esc(d.src)}" alt="${esc(d.alt)}" width="${d.width}" height="${d.height}" ` +
        `loading="lazy" class="w-full h-auto rounded-lg" />` +
  ...
  const pdf =
    `<p class="mt-5 text-sm"><a href="${esc(PO02_SPEC_PDF.href)}" ` +
    `class="font-semibold text-emerald-800 underline underline-offset-2">` +
    `${esc(PO02_SPEC_PDF.label)}</a></p>`;
...
// PO-06 (6 Sep 2026) - Portable Construction Site Cabin. The asset map declares the two
// technical diagrams under the numbered keys "01" and "02", the copy pack's spec groups
// use `headers` (plural, as PO-03's does), and the PDF label lives in the COPY pack's
// specifications_tab.pdf_link rather than on the asset map. Groups A-E render as the
// same premium grouped-table cards the porta-cabins Specs tab uses; every string comes
// from the signed pack and nothing is retyped. Each group's note renders under its own
// table, then the two diagrams, then the PDF link.
const PO06_IMG_ROOT = `/${constructionSiteCabinAssets.output_root.replace('public/', '')}`;
const PO06_SPEC_DIAGRAMS = (['01', '02'] as const).map((k) => ({
  src: `${PO06_IMG_ROOT}/${constructionSiteCabinAssets.spec_diagrams[k].out}`,
  // Ruling 5 - alt text comes only from copy.alt_text, keyed by the output path. The
  // package CSV repeats one alt per size and would fail the duplicate-alt DOM check.
  alt: (constructionSiteCabinCopy.alt_text.spec_diagrams as Record<string, string>)[
    constructionSiteCabinAssets.spec_diagrams[k].out
  ],
  width: 1600,
  height: 900,
}));
const PO06_SPEC_PDF = {
  href: `/${constructionSiteCabinAssets.spec_pdf.out.replace('public/', '')}`,
  label: constructionSiteCabinCopy.specifications_tab.pdf_link.label,
} as const;

function buildConstructionSiteCabinSpecificationsHtml(): string {
  const spec = constructionSiteCabinCopy.specifications_tab;
  ...
  const diagrams = PO06_SPEC_DIAGRAMS
    .map((d) => (
      `<figure class="mt-4 m-0 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">` +
        `<img src="${esc(d.src)}" alt="${esc(d.alt)}" width="${d.width}" height="${d.height}" ` +
        `loading="lazy" class="w-full h-auto rounded-lg" />` +
  ...
  const pdf =
    `<p class="mt-5 text-sm"><a href="${esc(PO06_SPEC_PDF.href)}" ` +
    `class="font-semibold text-emerald-800 underline underline-offset-2">` +
    `${esc(PO06_SPEC_PDF.label)}</a></p>`;
...
export function getProductTabsHtml(
  ...
  // SOC-01 (4 Sep 2026) — Groups A-E from the signed pack; Shipping is the SAME
  // shared freight component the live porta-cabins page renders (both trailer
  // ladders, both zone city tables, the free-delivery lines, ODC note and the
  // tentative-price disclaimer), called with no options so it stays byte-identical.
  if (pageSlug === 'small-office-cabin') {
    return {
      specificationsHtml: buildSmallOfficeCabinSpecificationsHtml(),
      shippingHtml: buildShippingHtml(),
    };
  }
  // PO-01 (4 Sep 2026) - Groups A-E from the signed pack; Shipping is the SAME shared
  ...
  // byte-identical. The generic two-paragraph panel this page shipped came from
  // variantData.shippingHtml, which the product data file no longer sets.
  if (pageSlug === 'readymade-office-cabin') {
    return {
      specificationsHtml: buildReadymadeOfficeCabinSpecificationsHtml(),
      shippingHtml: buildShippingHtml(),
    };
  }
  // PO-02 (4 Sep 2026) - Groups A-E from the signed pack; Shipping is the SAME shared
  // freight component the live porta-cabins page renders (both trailer ladders with
  // eighteen bands each, both zone city tables, the two free-delivery lines, the ODC
  // note and the tentative-price disclaimer), called with no options so it stays
  ...
  // PO-06 - same contract: the Shipping tab is the shared freight component called with
  // no options, so both trailer ladders (eighteen distance bands each, 100 to 1,000 km),
  // both zone city tables, the two free-delivery lines, the ODC note and the
  // tentative-price disclaimer are byte-identical to the design lock. There is no
  // shippingHtml key on this route's product JSON, which is what would override it.
