...
export const PORTABLE_OFFICE_LINK_MODULES: Record<string, InternalLinkModule> = {
  ...
  'prefabricated-office-cabins': {
    ...
    rows: [
      ...
      // SOC-01 (4 Sep 2026) — this row pointed at modern-office-cabin, which is now
      // 301'd into small-office-cabin. Its anchor ("the glazed contemporary build")
  ...
  'small-office-cabin': {
    ...
    rows: [
      ...
      // SOC-01 (4 Sep 2026) — the fourth row pointed at portable-office-container,
      // which the signed pack lists in links.never / explore_range.never_list. Row
      // removed rather than repointed; the remaining three are the approved cluster
      // destinations and all return 200.
    ],
  },
};
          ...
          <div className={GALLERY_TRACK_CLASS[thumbTrackCount] || 'grid grid-cols-5 gap-2'}>
            {heroImages.map((img, i) => (
              ...
              >
                ...
                {nonLcpImagesReady && <Image src={img.src} unoptimized={data.optimizeLocalGalleryImages ? false : shouldBypassOptimizer(img.src)} alt={data.productSlug === 'flat-pack-container-office' && !showVideo && i === activeImageIndex ? `Thumbnail ${i + 1}: ${img.alt}` : data.productSlug === 'shipping-container-office' || (data.productSlug === 'bess-container' && !showVideo && i === activeImageIndex) || (data.labelActiveThumbnailAlt && !showVideo && i === activeImageIndex) ? `Thumbnail ${i + 1}: ${img.alt}` : (isC04Product || isC08Product ? img.alt : (!showVideo && i === activeImageIndex ? '' : img.alt))} width={data.productSlug === 'flat-pack-container-office' || eagerActiveGalleryImages ? 1254 : 150} height={data.productSlug === 'flat-pack-container-office' || eagerActiveGalleryImages ? 1254 : 150} className="w-full h-full object-cover" loading={data.productSlug === 'flat-pack-container-office' || eagerActiveGalleryImages ? 'eager' : 'lazy'} decoding="async" sizes="(max-width: 1023px) 18vw, 80px" />}
              </button>
            ))}
