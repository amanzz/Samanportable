#!/usr/bin/env python3
"""CH-02 DIAGNOSTIC verifier - NOT the acceptance gate.

This is verify_ch02.py with SEVEN assertions mechanically corrected, and nothing
else changed. It exists to answer one question: behind the seven assertions the
page cannot satisfy, is any real page defect hiding? Every correction is marked
CH02-CORRECTION and names the original line number.

The shipped verify_ch02.py is left untouched and is the gate of record. Its output
is attached alongside this one.
"""
...
def main():
    ...
    # --- prices ---
    def indian(n):  # CH02-CORRECTION 1, original L96: en-IN grouping, as the platform emits
        s_, first = str(n)[-3:], str(n)[:-3]
        while first:
            s_, first = first[-2:] + "," + s_, first[:-2]
        return s_
    for k, v in COPY["price_display"].items():
        ck(indian(v['ex_gst']) in T, f"ex-GST price for {k} ({indian(v['ex_gst'])}) not on the page", 96)
    ...
    # CH02-CORRECTION 2, original L139: the shared freight component's actual wording
    for token in ("free delivery: within Bangalore city", "Greater Noida"):
    ...
    # CH02-CORRECTION 3, original L174: exclude the shared calculator credentials band,
    # which /product/porta-cabins renders identically and 3.11 forbids touching.
    page_srcs = [s for s in srcs if "/credentials/" not in s]
    ck(not any(s.lower().endswith(".png") or s.lower().endswith(".jpg") for s in page_srcs),
       "a PNG or JPG reached the page; every fetched image must be WebP", 174)
    # CH02-CORRECTION 4, original L175: count only <img> tags inside the page's own
    # gallery. The raw count is 4 on the design lock too: shared header logo preload +
    # <img>, and the slide-1 preload + <img> that 3.10 requires.
    gallery_high = [t for t in re.findall(r"<img\b[^>]*>", body, re.I)
                    if 'fetchpriority="high"' in t and "luxury-container-house" in t]
    ck(len(gallery_high) == 1, f"fetchpriority=high on {len(gallery_high)} gallery images, want 1", 175)
    # CH02-CORRECTION 5, original L176: assert the slide-1 preload itself. imagesrcset
    # requires Next's optimizer, which would re-encode the pack's pinned WebPs - 9.
    default_slide1 = next(s["output"] for s in ASSETS["slots"] if s["slot"] == "hero_gallery.20x10.slide1")
    ck(re.search(r'<link[^>]+rel="preload"[^>]+' + re.escape(default_slide1), body) is not None,
       "preload link for slide 1 of the default size missing", 176)
    for slot in ASSETS["slots"]:
        # CH02-CORRECTION 6, original L180: strip the repo-only "public/" segment
        u = base + ASSETS["public_dir"].replace("/public/", "/") + slot["output"]
        s, n = head_len(u)
    ...
    for b in blocks:
        ...
        # CH02-CORRECTION 7, original L193: recurse into mainEntity/breadcrumb, which is
        # where the shared route nests Product and BreadcrumbList on every page.
        def walk(node):
        ...
        walk(data)
    # CH02-CORRECTION 7 (cont.): walking the graph also surfaces each FAQ Question
    # node, so the four required entities are asserted as present rather than as the
    # exact set. Nothing outside the four plus Question is emitted.
    required = {"ItemPage", "Product", "BreadcrumbList", "FAQPage"}
    ck(required <= types and not (types - required - {"Question"}),
