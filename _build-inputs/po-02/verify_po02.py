#!/usr/bin/env python3
"""PO-02 Prefabricated Office Cabins verifier (SAMAN-105 generic verifier plus page-specific gates).
...
if src.startswith("http"):
    raw = urlopen(Request(src, headers={"User-Agent": "Mozilla/5.0 PO02-verify"})).read().decode("utf-8", "ignore")
else:
...
check("no source PNG served", ".png" not in " ".join(re.findall(r'src="([^"]+)"', doc)) or all("/images/products/prefabricated-office-cabins" not in s or s.endswith(".webp") for s in re.findall(r'src="([^"]+)"', doc)))
...
# ---------------------------------------------------------------- PO-02 page-specific gates
# A. six gallery slides per size, thirty-six in total
for slug, gal in amap.get("gallery_new", {}).items():
    if isinstance(gal, dict) and "slides" in gal:
        check(f"six gallery slides for {slug}", len(gal["slides"]) == 6, len(gal["slides"]))
check("thirty-six gallery slides in total",
      sum(len(g["slides"]) for g in amap["gallery_new"].values() if isinstance(g, dict) and "slides" in g) == 36)

# B. the three withdrawn sizes must not survive anywhere on the page
for bad in ["40×8", "20×20", "40×12", "40x8 ft", "20x20 ft", "40x12 ft", "9 Sizes", "Nine published dimensions"]:
    check(f"withdrawn size or claim absent: {bad}", bad not in text)

# C. this page is not a container product
check("container disambiguation present",
      "A container office is built from or like an ISO shipping container" in text)
check("lifting described as eye plates, not castings",
      "one-hole eye plates" in text or "one-hole lifting eye plate" in text)
check("no ISO casting claimed as this product's fitting",
      "ISO corner castings" not in text or "no ISO corner castings" in text)

# D. the panel-system differentiators must actually be on the page
for must in ["bolted spline joint", "transport break seam", "site joining strip", "module grid datum",
             "demountable wall cassette", "corner interface plate"]:
    check(f"panel-system term present: {must}", must.lower() in text.lower())

# E. no unsourced commercial promise
import re as _re
_n = _re.sub(r"(\d)\s*(?:-|\u2013|to)\s*(\d)", r"\1-\2", text)
for bad in ["5-year warranty", "five year warranty", "5-year structural warranty",
            "1-year finishing warranty", "12 days", "recommended occupancy"]:
    check(f"unsourced promise absent: {bad}", bad.lower() not in _n.lower())
check("approved warranty line present",
      "Warranty period and exclusions are confirmed in the final quotation" in text)

# F. the approved redirect must be live and land here in one hop
try:
    from urllib.request import build_opener, HTTPRedirectHandler
    class NoRedirect(HTTPRedirectHandler):
        def redirect_request(self, req, fp, code, msg, headers, newurl):
            raise Exception("%d %s" % (code, newurl))
    op = build_opener(NoRedirect)
    try:
        op.open("https://www.samanportable.com/product/prefabricated-houses/saman-prefab-office")
        check("saman-prefab-office 301 to this page (one hop)", False, "no redirect")
    except Exception as e:
        m = str(e)
        check("saman-prefab-office 301 to this page (one hop)",
              m.startswith("301") and copy["meta"]["canonical"] in m, m[:120])
except Exception as e:
    check("saman-prefab-office redirect check ran", False, e)
