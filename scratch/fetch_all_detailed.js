const https = require('https');
const fs = require('fs');
const path = require('path');

const WP_URL = 'blog.samanportable.com';
const WC_PATH = '/wp-json/wc/v3/products';
const CK = 'ck_8649e855e2c557f0a545b674bd5fb1eacc32ad59';
const CS = 'cs_d9b9536ff84a86871d7877481225b939cd4bacb2';

let allProducts = [];

function fetchPage(page) {
  const url = `https://${WP_URL}${WC_PATH}?consumer_key=${CK}&consumer_secret=${CS}&page=${page}&per_page=100`;
  console.log(`Fetching page ${page}...`);
  
  https.get(url, (res) => {
    let data = '';
    res.on('data', (chunk) => {
      data += chunk;
    });
    
    res.on('end', () => {
      try {
        const json = JSON.parse(data);
        if (!Array.isArray(json)) {
          console.error(`Error or invalid JSON on page ${page}:`, json);
          saveAndExit();
          return;
        }
        
        if (json.length === 0) {
          saveAndExit();
          return;
        }
        
        const mapped = json.map((product) => ({
          name: product.name,
          slug: product.slug,
          permalink: product.permalink,
          categories: product.categories.map(c => ({ name: c.name, slug: c.slug }))
        }));
        
        allProducts = allProducts.concat(mapped);
        
        if (json.length < 100) {
          saveAndExit();
        } else {
          fetchPage(page + 1);
        }
      } catch (err) {
        console.error('Parse error:', err);
        saveAndExit();
      }
    });
  }).on('error', (err) => {
    console.error('Fetch error:', err);
    saveAndExit();
  });
}

function saveAndExit() {
  const outputPath = path.join(__dirname, 'all_products_detailed.json');
  fs.writeFileSync(outputPath, JSON.stringify(allProducts, null, 2));
  console.log(`Completed fetching. Total products: ${allProducts.length}. Saved to ${outputPath}`);
}

fetchPage(1);
