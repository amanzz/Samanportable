...
def encode(src_rel, out_rel, width, q_start, slot, crop_ratio=None):
    ...
    if im.mode in ("RGBA", "LA", "P"):
        im = im.convert("RGB")
    if crop_ratio:
        # PO-05-R2 change 1/2: the gallery masters are 1:1 and these slots are not.
        # Take a CENTRED crop to the slot's ratio - full width, equal trim top and
        # bottom. This is the sanctioned case in the standing rule ("crop only to
        # reach the slot's ratio, never to reduce file size").
        rw, rh = crop_ratio
        cw, ch = im.size
        nh = int(round(cw * rh / rw))
        if nh < ch:
            top = (ch - nh) // 2
...
def main():
    ...
    # Pack amendment, 6 Sep 2026: the shared explorer panel is a fixed aspect-[4/3]
    # object-cover box - measured on the live design lock, whose own asset for this
    # slot is 1920x1440. The pack's earlier 16:9 claim is withdrawn. Ship a CENTRED
    # 4:3 crop, which trims far less of a 1:1 frame than 16:9 did and leaves the whole
    # unit inside the box. The declared width is 1600 px; render() never upscales, so
    # a 1254 px master lands at its native width.
    for size, f in AMAP["section3_images"]["files"].items():
        rows.append(encode(f["src"], f["out"], rules["section3_image"]["width_px"],
                           rules["section3_image"]["quality_start"], "section3/" + size,
                           crop_ratio=(4, 3)))
